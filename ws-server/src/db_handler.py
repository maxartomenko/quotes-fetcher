...
async def get_quotes_for_period(
    *,
    ...
    try:
        ...
        """,
            parameters={"assetId": asset_id, "minutes": period_minutes},
        )
        return [
            AssetQuoteResponse(time=row[0].timestamp(), assetId=asset_id, value=row[1])
...
async def get_asset_by_id(
    *, clickhouse_client: clickhouse_connect.driver.AsyncClient, asset_id: int
) -> Asset | None:
    try:
        query_result = await clickhouse_client.query(
        ...
        if query_result.result_rows:
            ...
            return Asset(_id=int(row[0]), name=row[1])
    except ClickHouseError as e:
